...
gcloud run deploy "$SERVICE" \
  ...
  --project "$PROJECT" \
  --region "$REGION" \
  --set-env-vars "^@^GOOGLE_GENAI_USE_VERTEXAI=TRUE@GOOGLE_CLOUD_PROJECT=$PROJECT@GOOGLE_CLOUD_LOCATION=${GOOGLE_CLOUD_LOCATION:-global}@GEMINI_MODEL=${GEMINI_MODEL:-gemini-3.6-flash}@FIRESTORE_DATABASE=${FIRESTORE_DATABASE:-(default)}@INTAKE_ALLOWED_ORIGINS=${INTAKE_ALLOWED_ORIGINS:-http://localhost:5173}@INTAKE_TRACING=${INTAKE_TRACING:-0}" \
